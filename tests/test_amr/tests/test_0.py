...
def test_0(dir_name = 'test_amr'):
    ...
    for trial in range(0, ntrial):
        ...
        u_vec      = merge_vecs(intr_mask, u_vec_intr, bcs_vec)
                ...
                for F in range(0, 4):
                    if (F%2 == 0):
                        ...
                        for jj in range(0, ndof_y):
                            err = np.abs(col_intg_th[F][jj] - anl_sol_intg_th(xxf[x_idx], yyf[jj]))
                        ...
                        for ii in range(0, ndof_x):
                            err = np.abs(col_intg_th[F][ii] - anl_sol_intg_th(xxf[ii], yyf[y_idx]))
                            max_err = max(max_err, err)
