    ...
    for ref_strat_name, _ in refinement_strategies.items():
        ...
        for trial_dir_path in trial_dir_paths:
            ...
            if ((float(ndof) / float(prev_ndof) >= ndof_output_ratio)
                ...
                # Spatial-jump error indicator
                err_ind_spt_jmp_file_name: str = "err_ind_spt_jmp.json"
                err_ind_spt_jmp_file_path: str = os.path.join(trial_dir_path,
                                                              err_ind_spt_jmp_file_name)
                if os.path.isfile(err_ind_spt_jmp_file_path):
                    err_ind_spt_jmp: Error_Indicator = error_indicator_from_file(mesh_file_path,
                                                                                 uh_file_path,
                                                                                 err_ind_spt_jmp_file_path)

                    err_ind_spt_jmp_plot_file_name: str = "err_ind_spt_jmp.png"
                    err_ind_spt_jmp_plot_file_path: str = os.path.join(trial_dir_path,
                                                                       err_ind_spt_jmp_plot_file_name)
                    if not os.path.isfile(err_ind_spt_jmp_plot_file_path):
                        plot_refinement_thresholds(err_ind_spt_jmp,
                                                   err_ind_spt_jmp_plot_file_path,
                                                   kind = "spt")
