...
def main():
    ...
    if seed == 0:
        ...
        seed: int = mpi_comm.bcast(seed, root = consts.COMM_ROOT)

    ## Calculate, output, and plot the solution
    for ref_strat_name, ref_strat in refinement_strategies.items():
        seed: int = seed + 1
        rng: np.random._generator.Generator = np.random.default_rng(seed)
        if comm_rank == consts.COMM_ROOT:
        ...
        ## Loop through trials, i.e., iterations of the mesh refinement
        while (keep_going):
            if comm_rank == consts.COMM_ROOT:
            ...
            if comm_rank == consts.COMM_ROOT:
                ...
                err_ind_ang: Error_Indicator = Error_Indicator(uh, **ref_strat["ang"])
                err_ind_ang.error_angular_jump()

                ## Save the angular jump error indicator to file
                err_ind_ang_file_name: str = "err_ind_ang_jmp.json"
                err_ind_ang_file_path: str = os.path.join(trial_dir_path, 
                                                          err_ind_ang_file_name)
                err_ind_ang.to_file(err_ind_ang_file_path, 
                                        write_projection = False, 
                                        write_mesh = False)
                
                ## Calculate the spatial jump error indicator
                err_ind_spt: Error_Indicator = Error_Indicator(uh, **ref_strat["spt"])
                err_ind_spt.error_spatial_jump()

                ## Save the spatial jump error indicator to file
                err_ind_spt_file_name: str = "err_ind_spt_jmp.json"
                err_ind_spt_file_path: str = os.path.join(trial_dir_path, 
                                                          err_ind_spt_file_name)
                err_ind_spt.to_file(err_ind_spt_file_path, 
                                    write_projection = False, 
                                    write_mesh = False)
            ...
            if comm_rank == consts.COMM_ROOT:
                ## Angular-spatial steering criterion
                ang_EA2R: float = err_ind_ang.error_to_resolve
                spt_EA2R: float = err_ind_spt.error_to_resolve
                ang_spt_thrsh: float = spt_EA2R / (spt_EA2R + ang_EA2R)

                steer: float = rng.random()
                if (steer < ang_spt_thrsh): # Refine spatially
                    ref_kind: str = "spt"
                    l_mesh: Mesh = err_ind_spt.ref_by_ind()
                else: # Refine angularly
                    ref_kind: str = "ang"
                    l_mesh: Mesh = err_ind_ang.ref_by_ind()

                ref_strat_log[trial]["ref_kind"] = ref_kind
