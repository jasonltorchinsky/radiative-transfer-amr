...
def ref_mesh(ref_strat, mesh, uh_proj, comm_rank, rng, do_plot_err_ind, trial,
    ...
    if do_plot_err_ind and (trial%5 == 0 or do_calc_err):
        ...
        hr_err_kwargs: dict = {"ref_kind"   : "ang",
                               "ang_res_offset" : 3,   # Factor to add to angular DoFs
                               "spt_res_offset" : 0,   # Factor to add to spatial DoFs
                               "verbose"    : True, # Print info while executing
                               "blocking"   : True, # Synchronize ranks before exiting
                               "out_dir"    : trial_dir
                               }
        hr_err_ind = amr.high_res_err(mesh, uh_proj, 
                                      params.kappa, params.sigma,
                                      params.Phi, params.bcs_dirac, 
                                      params.f, **hr_err_kwargs)
        gen_err_ind_plot(mesh, hr_err_ind, trial, trial_dir, "hr_err_ang.png")
