...
def main():
    ...
    for ref_strat_name, _ in refinement_strategies.items():
        ...
        if os.path.isfile(tracked_values_file_path):
            tracked_values_file = open(tracked_values_file_path)
            tracked_values_dict: dict = json.load(tracked_values_file)
            tracked_values_file.close()

            ndofs: np.ndarray = np.ndarray(tracked_values_dict["ndof"].values())
            errs: np.ndarray = np.ndarray(tracked_values_dict["error"].values())

            for ii in np.size(ndofs):
                ref_strat_err_dict[ndofs[ii]] = errs[ii]
        else:
            prev_ndof: int = 1
    ...
    for ref_strat_name, err_dict in tracked_values.items():
        ndofs: np.ndarray  = np.array(list(err_dict.keys()))
