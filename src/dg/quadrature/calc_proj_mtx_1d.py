...
def calc_proj_mtx_1d(src_nodes: np.ndarray, trgt_nodes: np.ndarray) -> np.ndarray:
    """
    ...
    n_src: int = np.shape(src_nodes)[0]
    n_trgt: int = np.shape(trgt_nodes)[0]
    ...
    if (n_src > n_trgt):
        P: np.ndarray = np.zeros([n_src, n_trgt])
        ...
        P: np.ndarray = np.zeros([n_trgt, n_src])
        for jj in range(0, n_trgt):
            for ii in range(0, n_src):
