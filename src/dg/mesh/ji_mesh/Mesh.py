...
class Mesh:
    ...
    def ref_col(self, col, kind = 'all'):
        """
        Refine a column, spatially ('spt'), angularly ('ang'), or both ('all').
        """
    ...
    def ref_mesh(self, kind = 'all'):
        ...
        if (kind == 'ang') or (kind == 'all'):
            for col_key in col_keys:
                self.ref_col(self.cols[col_key], kind = 'ang')

        if (kind == 'spt') or (kind == 'all'):
            for col_key in col_keys:
                self.ref_col(self.cols[col_key], kind = 'spt')
