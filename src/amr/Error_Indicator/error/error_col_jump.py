...
def error_col_jump(self) -> None:
    ...
    for col_key_0, col_0 in col_items:
        ...
        for col_face in range(0, 4):
            ...
            for col_key_1 in nhbr_keys:
                ...
                for cell_key_0, cell_0 in cell_items_0:
                    ...
                    # Get nhbr_cell info
                    nhbr_keys: list = self.nhbr_cell_in_nhbr_col(col_key_0,
                                                                 cell_key_0,
                                                                 col_key_1)
                    for cell_key_1 in nhbr_keys:
                        assert(cell_key_1 is not None)
        ...
        col_err: float = (1. / (perim * 2. * np.pi)) * col_jump
        mesh_err += col_err

        cols[col_key] = Error_Indicator_Column(np.sqrt(col_err))
