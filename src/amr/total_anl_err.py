...
def total_anl_err(mesh, proj, anl_sol, **kwargs):
    return total_anl_err_hr(mesh, proj, anl_sol, **kwargs)

def total_anl_err_lr(mesh, proj, anl_sol, **kwargs):
    """
...
def total_anl_err_hr(mesh, proj, anl_sol, **kwargs):
    """
    Calculate the L2-error by cell (and column), weighted to be the relative error.
    """
    ...
    for col_key, col in col_items:
        if col.is_lf:
            ...
            xxf_hr = push_forward(x0, x1, xxb_hr).reshape([ndof_x_hr, 1, 1])
            yyf_hr = push_forward(y0, y1, yyb_hr).reshape([1, ndof_y_hr, 1])
            ...
            for cell_key, cell in cell_items:
                if cell.is_lf:
                    ...
                    thf_hr = push_forward(th0, th1, thb_hr).reshape([1, 1, ndof_th_hr])
