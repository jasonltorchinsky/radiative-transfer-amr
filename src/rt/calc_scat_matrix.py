...
import consts
from dg.matrix import get_idx_map, get_cell_idxs, get_col_idxs
...
def calc_scat_matrix(mesh: Mesh, sigma: Callable[[np.ndarray, np.ndarray, np.ndarray], 
                                                 np.ndarray],
            ...
            if cell_0.is_lf:
                ...
                for cell_key_1, cell_1 in cell_items:
                    if cell_1.is_lf:
                        ...
                        alphalist = np.zeros([cell_ndof], dtype = consts.INT)
                        betalist  = np.zeros([cell_ndof], dtype = consts.INT)
