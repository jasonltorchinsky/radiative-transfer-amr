...
def calc_scat_matrix_new(mesh, sigma, Phi, **kwargs):
    ...
    mesh_iter = itertools.repeat(mesh, times = ncols)
    sigma_iter = itertools.repeat(sigma, times = ncols)
    Phi_iter = itertools.repeat(Phi, times = ncols)
    arg_iter = itertools.zip_longest(mesh_iter, mesh.cols.keys(),
                                     sigma_iter, Phi_iter)
    
    p = Pool(32)
    col_mtxs_list = p.starmap(get_col_matrix, arg_iter)
    p.close()
    
    for col_key, col_mtx in col_mtxs_list:
...
def get_col_matrix(mesh, col_key, sigma, Phi):
