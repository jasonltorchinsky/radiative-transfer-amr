...
def preconditioner_matrix(self, mesh: Mesh, **kwargs) -> list:
    """
    ...
    default_kwargs: dict = {"return_preconditioner" : False, # Return preconditioner matrix
    ...
    if kwargs["verbose"]:
        ...
        msg: str = ( "Constructing (Extinction - Scattering) Matrix...\n" )
        utils.print_msg(msg)
    ...
    for col_key in col_keys_local:
        [col_mtx, inv_col_mtx] = calc_col_matrix(mesh, col_key, self.kappa,
                                                 self.sigma, self.Phi,
                                                 kwargs["return_preconditioner"])
...
def calc_col_matrix(mesh: Mesh, col_key: int, kappa: Callable[[np.ndarray, np.ndarray, np.ndarray], 
                    ...
                    Phi: Callable[[np.ndarray, np.ndarray, np.ndarray], 
                                  np.ndarray], return_preconditioner: bool = False):
    ...
    if col.is_lf:
        ...
        wx_wy_kappa_h: np.ndarray = wx * wy * kappa(xxf, yyf)
        ...
        for cell_key_0, cell_0 in cell_items:
            ...
            for cell_key_1, cell_1 in cell_items:
                ...
                if cell_key_0 != cell_key_1:
                    ...
                    for ii in range(0, nx):
                        for jj in range(0, ny):
                            wx_wy_kappa_ij: float = wx_wy_kappa_h[ii, jj]
                            wx_wy_sigma_ij: float = wx_wy_sigma_h[ii, jj]
                            for rr in range(0, nth_0):
                                wth_0_rr: float = wth_0[rr]
                                for aa in range(0, nth_1):
                                    ...
                                    if rr == aa:
                                        val: float = dcoeff * (wth_1_aa * wx_wy_kappa_ij # Extinction term
                                                        - (dth_1 / 2.0) * wx_wy_sigma_ij * wth_0_rr * wth_1_aa * Phi_ra) # Scattering term
                                    else:
                                        val: float = -dcoeff * (dth_1 / 2.0) * wx_wy_sigma_ij * wth_0_rr * wth_1_aa * Phi_ra # Scattering term
        ...
        if return_preconditioner:
            inv_col_mtx: sp._csr.csr_matrix = spla.inv(col_mtx.tocsr())
        else:
            inv_col_mtx: sp._csr.csr_matrix = None
